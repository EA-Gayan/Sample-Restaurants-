       ...
       className=' w-10 min-w-[40px] h-10 min-h-[40px] drop-shadow-xl cursor-pointer rounded-full' 
        ...
        onClick={login}/>
        {
          isMenu && (
            ...
            animate={{opacity: 8, scale:1}}
            exit={{opacity: 0, scale:0.5}}
            className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute right-1'>
            {
              user && user.email === "hipet2022@gmail.com" && (
                <Link to = {"/createItem"}>
      ...
      <div className='relative'>
        ...
        onClick={login}/>
        {
          isMenu && (
            ...
            <p className='m-2 p-2 rounded-md shadow-md flex items-center justify-center gap-3 cursor-pointer hover: bg-gray-300 transition-all duration-100 ease-in-out text-textColor text-base' onClick={logout}>Logout <MdLogout/>
            </p>
          </motion.div>
          )
        }
